...
function initMarquee() {
	...
	const totalUniqueImages = 18; // Number of unique images per row

	// Array of 18 unique placeholder image sources
	const imageSources = Array.from(
		{ length: totalUniqueImages },
		(_, i) => `images/meme/${String(i + 1).padStart(1)}.avif`
	);
...
const memeImages = Array.from({ length: 20 }, (_, i) => `images_${i + 1}.avif`);
		...
		noImagesSection.innerHTML = `
            <div class="image-container" style="display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%);">
                <div style="text-align: center; color: white; padding: 2rem;">
                    ...
                    <p style="font-size: 0.9rem; color: #888;">Expected format: images_1.avif, images_2.avif, etc.</p>
